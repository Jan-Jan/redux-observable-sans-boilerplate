...
export const rpcFailure = (type, url, httpType) => response =>
  ([
    { type: `${type}_FAILURE`, payload: response.xhr ? response.xhr.response : response.body },
    {
      type: response.code === 401 ? `${httpType}_UNAUTHORIZED` : `${httpType}_ERROR`,
    ...
    action$.pipe(
      ...
      mergeMap(({ payload }) => {
        ...
        const sideEffect$ = from(fetch(url, options).then(responseToJson)).pipe(
          ...
          catchError(mergeMap(from(rpcFailure(type, url, httpType))),
        )
...
export const httpAction = id => (type, url, method = 'GET', settings = {}) => body =>
  ({
    type: createType(id),
