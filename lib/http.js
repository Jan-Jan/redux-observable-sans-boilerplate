...
const mapResponseToSuccessOrFailureStream = (type, url) => response => {
  if (response.code >= 200 && response.code < 300) {
    return from([rpcSuccess(type)(response.body)])
  }
  // handle error
  const failure = rpcFailure(type)(response)
  const unauthorized = { type: `UNAUTHORIZED`, payload: { url, ...response } }
    ...
    action$.pipe(
      ...
      mergeMap(({ payload }) => {
        ...
        const sideEffect$ = from(fetch(url, options).then(responseToJson)).pipe(
          switchMap(mapResponseToSuccessOrFailureStream(type, url)),
