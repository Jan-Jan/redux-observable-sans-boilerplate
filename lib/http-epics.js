...
import { concat, merge, Observable, from, race } from 'rxjs'
...
export const jsonEpic = options => {
    ...
    const httpType = `HTTP_JSON` + (options.id ? `_${id}` : ``)
    ...
    return (action$, state$, deps) =>
        action$.pipe(
            ...
            mergeMap(({ payload }) => {
                ...
                //return from(new Promise(resolve => resolve({ type: 'p' })))

                return from([{ code: 401, body: { wat: 'WOOT' }}])
                .pipe(
                ...
                return race(concat(reqAction$, sideEffect$), hotReloading$)
