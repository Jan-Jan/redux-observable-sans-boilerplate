...
describe('httpEpic', () => {
  ...
  it('basic GET', done => {
    ...
    // setup
    const httpAct = httpAction(dispatch)(type, path, method)()
  ...
  it('POST with auth headers and id', done => {
    ...
    // setup
    const httpAct = httpAction(dispatch, id)(type, path, method)(body)
    const expectedActions = [
  ...
  it('401 with extra action', done => {
    ...
    const expectedActions = [
      ...
      { type: `HTTP_LH_UNAUTHORIZED`, payload: { url: `${url}/${path}`, code: response.status, body: response.body } },
    ]
    fetchMock.mock({
      matcher: `${url}/${path}`,
      method,
    ...
    // run
    const result$ = httpEpic({ url, id })(action$, state$)
    result$.subscribe(testOutput(done, testFetch, expectedActions), console.error, console.error)
    subject.next(httpAct)
  })
  ...
  it('http error', done => {
    ...
    const method = `POST`
    const body = { what: 'evs' }
    const response = { status: 500, body: { error: 'no entry' } }
    // setup
    const httpAct = httpAction(dispatch, id)(type, path, method)(body)
    const expectedActions = [
      { type: `${type}_REQUEST`, payload: { body, method, url: `${url}/${path}` } },
      { type: `${type}_FAILURE`, payload: response.body },
      { type: `HTTP_LH_ERROR`, payload: { url: `${url}/${path}`, code: response.status, body: response.body } },
    ]
    fetchMock.mock({
      matcher: `${url}/${path}`,
      method,
  ...
  it('throws', done => {
    ...
    const method = `POST`
    const body = { what: 'evs' }
    const msg = 'none of that'
    const response = () => { throw new Error(msg) }
    // setup
    const httpAct = httpAction(dispatch)(type, path, method)(body)
    const expectedActions = [
      { type: `${type}_REQUEST`, payload: { body, method, url: `${url}/${path}` } },
      { type: `${type}_FAILURE` },
      { type: `HTTP_ERROR`, payload: { url: `${url}/${path}` } },
    ]
    fetchMock.mock({
      matcher: `${url}/${path}`,
      method,
